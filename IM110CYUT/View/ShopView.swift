...
struct RecipeView: View
{
                ...
                ForEach(Array(recipes.enumerated()), id: \.element.name) { index, recipe in
                    ForEach(recipe.ingredients.indices, id: \.self) { ingredientIndex in
                        ...
                        HStack
                        {
                            ...
                            // 數量輸入框
                            // 在 TextField 中添加 .onSubmit 來監聽回車事件
                            TextField("數量", text: $recipes[index].ingredients[ingredientIndex].quantityInput)
                                ...
                                .onSubmit {
                                    ...
                                    // 清空數量輸入框
                                    recipes[index].ingredients[ingredientIndex].quantityInput = ""
                                }

                               

                            // 選取標誌
                            Image(systemName: selectedIngredient?.id == ingredient.id ? "checkmark.square" : "square")
    ...
    private func separateQuantityAndUnit(_ input: String) -> (quantity: String, unit: String)
    {
        ...
        // 找到數量部分
        let quantity = input.trimmingCharacters(in: numericCharacterSet.inverted)
        
        // 找到單位部分
        let unit = input.trimmingCharacters(in: numericCharacterSet)
    ...
    private func updateQuantity(for recipeIndex: Int, and ingredientIndex: Int)
    {
        ...
        {
            // 分離數量和單位
            let separatedValues = separateQuantityAndUnit(recipes[recipeIndex].ingredients[ingredientIndex].quantityInput)
            
            // 分離原始數量和單位
            let currentValues = separateQuantityAndUnit(recipes[recipeIndex].ingredients[ingredientIndex].quantity)
            
            // 更新數量，保留原來的單位
            recipes[recipeIndex].ingredients[ingredientIndex].quantity = "\(separatedValues.quantity)" + currentValues.unit
        }
    }
                    ...
                    onDeleteIngredient: { ingredient in
                        ...
                        if let recipeIndex = shoppingList.recipes.firstIndex(where: { $0.ingredients.contains(where: { $0.id == ingredient.id }) }) {
                            if let ingredientIndex = shoppingList.recipes[recipeIndex].ingredients.firstIndex(where: { $0.id == ingredient.id }) {
                                ...
                                // 如果食譜中沒有其他食材，則刪除整個食譜
                                if shoppingList.recipes[recipeIndex].ingredients.isEmpty {
