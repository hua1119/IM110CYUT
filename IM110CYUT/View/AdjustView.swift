...
struct RecipeItemView: View
{
    ...
    var body: some View
    {
        ...
        else
        {
            List
            {
                ForEach(Array(recipes.enumerated()), id: \.element.name)
                { index, recipe in
...
struct AdjustView: View
{
    ...
    func addCustomRecipe(_ newRecipe: CustomRecipe)
    {
        ...
        {
            for newIngredient in newRecipe.ingredients // 食譜已存在，將其食材進行疊加
            {
                if let existingIngredientIndex = shoppingList.recipes[existingIndex].ingredients.firstIndex(where: { $0.name == newIngredient.name })
                {
                    shoppingList.recipes[existingIndex].ingredients[existingIngredientIndex].quantity += newIngredient.quantity // 食材已存在，進行數量疊加
                }
                else // 食材不存在，直接添加
                {
